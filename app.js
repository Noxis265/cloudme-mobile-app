...
function renderAvatar(profile,sizeClass=''){
  ...
  return`<div class="avatar-wrap ${sizeClass}">${av||'☁️'}</div>`;
}
function renderAvatarWithDot(profile,sizeClass='',showDot=false){
  ...
  return`<div class="avatar-wrap ${sizeClass}" style="position:relative">${av||'☁️'}${dot}</div>`;
}
...
function renderTopbar(){
  return`<div class="topbar">
    <div class="logo"><span>☁️</span>CloudMe</div>
    <div class="navgroup">
      <button class="navbtn ${state.view==='directory'?'active':''}" data-action="nav-directory">Сеть</button>
      <button class="navbtn ${state.view==='chats'||state.view==='new-group'?'active':''}" data-action="nav-chats">Чаты</button>
...
function renderDirectory(){
  ...
  if(state.busy&&!state.directoryLoaded)body=`<div class="loading-state">Загружаем…</div>`;
    ...
    if(q.length>=2){
      ...
      body+=`<div class="empty-state">Введи имя друга в поиск, чтобы найти его и начать переписку.</div>`;
    }
  }
  ...
  return`<div class="panel"><h2>Мой профиль</h2>
    <form data-form="profile">
      <div class="avatar-picker-section">
        ...
        <div class="emoji-grid">${EMOJIS.map(e=>`<button type="button" class="emoji-btn ${e===selectedEmoji&&!isPhoto?'selected':''}" data-emoji="${e}">${e}</button>`).join('')}</div>
      </div>
...
function renderNewGroup(){
  ...
  return`<div class="panel"><h2>Новая группа</h2>
    ...
    <div class="emoji-grid" style="grid-template-columns:repeat(6,1fr);margin-bottom:18px;">
      ${GROUP_EMOJIS.map(e=>`<button type="button" class="emoji-btn ${e===selectedGroupEmoji?'selected':''}" data-action="pick-group-emoji" data-emoji="${e}">${e}</button>`).join('')}
    </div>
...
function reactionsHtml(msg){
  ...
  const pills=Object.entries(totals).map(([emoji,count])=>{
    ...
    return`<div class="reaction-pill ${mine?'mine':''}" data-action="toggle-reaction" data-msg-id="${msg.id}" data-emoji="${emoji}">${emoji}<span class="count">${count}</span></div>`;
  });
...
function renderMessageBubble(msg,idx){
  ...
  return`<div class="msg-row ${isMe?'me':''}" data-msg-id="${msg.id}" data-kind="${state.activeConversation?state.activeConversation.kind:'dm'}" data-conv-id="${state.activeConversation?state.activeConversation.id:''}">
    ...
    <div class="msg-bubble ${isCurrent?'highlight':''} ${isPinned?'pinned':''}" ${isEditing?'':'data-action="open-reaction-picker"'} data-msg-id="${msg.id}">
      ${isEditing?'':'<div class="msg-react-hint">Реакция · ответ · 📌</div>'}
      ${isPinned?'<div class="msg-pin-flag">📌 закреплено</div>':''}
      ${!isMe&&isGroup?`<div class="msg-sender">${esc(msg.senderName||'')}</div>`:''}
      ${bodyHtml}
      <div class="msg-time">${timeAgo(msg.createdAt)}${msg.edited?' · изменено':''}</div>
      ${reactionsHtml(msg)}
    </div>
  </div>`;
}
...
function openReactionPicker(msgId,bubbleEl){
  ...
  picker.innerHTML=REACTION_EMOJIS.map(e=>`<button data-action="pick-reaction" data-emoji="${e}" data-msg-id="${msgId}">${e}</button>`).join('')
    +`<button data-action="reply-to-message" data-msg-id="${msgId}" title="Ответить">↩️</button>`
    +`<button data-action="${isPinned?'unpin-message':'pin-message'}" data-msg-id="${msgId}" title="${isPinned?'Открепить':'Закрепить'}">📌</button>`
    +(isMe?`<button data-action="start-edit-message" data-msg-id="${msgId}" title="Редактировать">✏️</button><button data-action="delete-message" data-msg-id="${msgId}" title="Удалить">🗑</button>`:'');
  document.body.appendChild(picker);
  const rect=bubbleEl.getBoundingClientRect();
  let top=rect.top-52,left=rect.left;
  if(top<8)top=rect.bottom+6;if(left+220>window.innerWidth)left=window.innerWidth-228;
...
function openConversation(peerUid,peerName,peerEmoji){
  ...
  state.searchOpen=false;state.searchQuery='';state.searchResults=[];state.searchIdx=0;state.pinnedMsgs=[];state.peerTyping=false;
  state.view='chats';render();subscribeThread();subscribeConvMeta();
}
function openGroup(g){
  state.activeConversation={kind:'group',id:g.id,name:g.name,emoji:g.emoji,members:g.members};
...
function subscribeGroupList(){
  ...
  groupListUnsub=db.collection('groups').where('members','array-contains',state.currentUid).onSnapshot(snap=>{
    const list=[];
    snap.forEach(doc=>{const d=doc.data();list.push({id:doc.id,name:d.name||'Группа',emoji:d.emoji||'👥',members:d.members||[],lastMessage:d.lastMessage||'',lastMessageAt:d.lastMessageAt,pinned:d.pinned||[]});});
